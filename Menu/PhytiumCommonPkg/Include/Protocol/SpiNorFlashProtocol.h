...
typedef
EFI_STATUS
(EFIAPI *NORFLASH_PLATFORM_WRITE_PROTECT_INTERFACE) (
  IN BOOLEAN             Enable,
  IN UINT32              BpBit
  );
...
typedef
VOID
(EFIAPI *NORFLASH_PLATFORM_WRITE_PROTECT_PAR_CONFIG_INTERFACE) (
  IN  UINT8    Enable,
  IN  UINT8    WpBit,
  IN  UINT16   Delay
  );
...
struct _EFI_NORFLASH_DRV_PROTOCOL{
  ...
  NORFLASH_PLATFORM_WRITE_PROTECT_INTERFACE    WriteProtect;
  NORFLASH_PLATFORM_WRITE_PROTECT_PAR_CONFIG_INTERFACE  WriteProtectParConfig;
};
