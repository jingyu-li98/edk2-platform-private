  ...
  )
{
  ...
  //DEBUG((EFI_D_INFO,"%a(),%d,HorizontalResolution:%d\n",__FUNCTION__,__LINE__,HorizontalResolution));
  //DEBUG((EFI_D_INFO,"%a(),%d,VerticalResolution:%d\n",__FUNCTION__,__LINE__,VerticalResolution));
  ...
  //DEBUG((EFI_D_INFO,"%a(),%d,MaxColumns:%d\n",__FUNCTION__,__LINE__,MaxColumns));
  //DEBUG((EFI_D_INFO,"%a(),%d,MaxRows:%d\n",__FUNCTION__,__LINE__,MaxRows));
  ...
  //DEBUG((EFI_D_INFO,"ValidCount:%d,Columns:%d,Rows:%d,DeltaX:%d,DeltaY:%d,GopWidth:%d,GopHeight:%d\n",
    //ValidCount,NewModeBuffer[ValidCount].Columns,NewModeBuffer[ValidCount].Rows,NewModeBuffer[ValidCount].DeltaX,NewModeBuffer[ValidCount].DeltaY,
    //NewModeBuffer[ValidCount].GopWidth,NewModeBuffer[ValidCount].GopHeight));
  ValidCount++;

  if ((MaxColumns >= 80) && (MaxRows >= 50)) {
    NewModeBuffer[ValidCount].Columns = 80;
    NewModeBuffer[ValidCount].Rows    = 50;
    NewModeBuffer[ValidCount].DeltaX  = (HorizontalResolution - (MaxColumns * EFI_GLYPH_WIDTH)) >> 1;
    NewModeBuffer[ValidCount].DeltaY  = (VerticalResolution - (MaxRows * EFI_GLYPH_HEIGHT)) >> 1;
  }
  NewModeBuffer[ValidCount].GopWidth      = HorizontalResolution;
  NewModeBuffer[ValidCount].GopHeight     = VerticalResolution;
  NewModeBuffer[ValidCount].GopModeNumber = GopModeNumber;
  ...
  )
{
  ...
  //DEBUG((EFI_D_INFO,"%a(),%d,HorizontalResolution:%d,VerticalResolution:%d\n",__FUNCTION__,__LINE__,HorizontalResolution,VerticalResolution));
  ...
  //DEBUG((EFI_D_INFO,"%a(),%d,DefaultColumn:%d,DefaultRow:%d\n",__FUNCTION__,__LINE__,DefaultColumn,DefaultRow));
  Column = 0;
  Row = 0;
  ...
  )
{
  ...
  //DEBUG((EFI_D_INFO,"%a(),%d,MaxColumn:%d,MaxRow:%d,DeltaX:%d,DeltaY:%d,Width:%d,Height:%d\n",
    //__FUNCTION__,__LINE__,MaxColumn,MaxRow,DeltaX,DeltaY,Width,Height));
...
{
  EFI_TPL               OldTpl;
  //del return for supporting wide ,by liyu 20200327
  ...
  )
{
  ...
  Location = Location + mNarrowFontSize;
