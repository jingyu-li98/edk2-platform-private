...
CHAR16            *gF9HelpString;
CHAR16            *gF10HelpString;
  ...
  )
{
  ...
  while (!IsNull (&FormData->HotKeyListHead, Link)) {
    ...
    //zhjxl use to replace FormBrowserEx2->RegisterHotKey FUNCTION_NINE_STRING FUNCTION_TEN_STRING string
    //here to display chinese string
    if(HotKey->KeyData->ScanCode == SCAN_F9 )
      ColumnStr = gF9HelpString;
    if(HotKey->KeyData->ScanCode == SCAN_F10 )
      ColumnStr = gF10HelpString;
  ...
  )
{
  ...
  gF9HelpString         = LibGetToken (STRING_TOKEN (FUNCTION_NINE_STRING), mCDLStringPackHandle);
  gF10HelpString        = LibGetToken (STRING_TOKEN (FUNCTION_TEN_STRING), mCDLStringPackHandle);
  ...
  )
{
  ...
  FreePool (gF9HelpString);
  FreePool (gF10HelpString);
  ...
  )
{
  ...
  if (Buffer[Index] >= 0x4e00 && Buffer[Index] <= 0x9fa4){
  ...
  do {
    for (; (Buffer[Index] != NARROW_CHAR) && (Buffer[Index] != WIDE_CHAR) && (Buffer[Index] != 0); Index++) {
      ...
      if(((Buffer[Index]<0x4e00 || Buffer[Index]>0x9fa4) && (Buffer[Index+1]>=0x4e00 && Buffer[Index+1]<=0x9fa4)) ||
          ((Buffer[Index+1]<0x4e00 || Buffer[Index +1]>0x9fa4) && (Buffer[Index]>=0x4e00 && Buffer[Index]<=0x9fa4))) {
        break;
      }
    }
    ...
    //
    if (((Buffer[Index] == NARROW_CHAR)&&(Buffer[Index+1]<0x4e00 || Buffer[Index +1]>0x9fa4))|| // \narrowAAA
        ((Buffer[Index+1]<0x4e00 || Buffer[Index +1]>0x9fa4) && (Buffer[Index]>=0x4e00 && Buffer[Index]<=0x9fa4)) // AAA
        ) {
      //
