  ...
  )
{
  ...
  if ((gLibIsFirstForm) || ((FormData->Attribute & HII_DISPLAY_MODAL) != 0)) {
    ...
    ClearLines (0, gScreenDimensions.RightColumn, gScreenDimensions.BottomRow - 4, gScreenDimensions.BottomRow - 3, KEYHELP_BACKGROUND);
  ...
  )
{
  ...
  case INPUT_ERROR:
    if (State) {
      ...
      gST->ConOut->SetAttribute (gST->ConOut, KEY_BACKGROUND);
