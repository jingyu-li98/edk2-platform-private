...
#include "FrontPage.h"
#include <Library/EncryptionLib.h>
...
#define UI_HII_DRIVER_LIST_SIZE               0x10
...
//"en-US;zh-Hans"
//   0      1
UINT8                        gDefaultLanguageIndex = 1;
BOOLEAN                      mFirstEnterFrontPage = FALSE;
...
/**
  ...
  @param[in,out]  LangCode      On input: point to first language in the list. On
  ...
  )
{
  ...
  if (Index == Value->u8) {
    ...
    //
    // N/ESC need update Front Page to Reduction
    //
    UpdateFrontPageForm ();
...
/**
  Check Password after press "Enter Setup".

  @retval       EFI_SUCCESS    Success.
                Other          Old password not exist.
...
)
{
  ...
  if (Status == EFI_SUCCESS) {
    PasswordDecode(&PasswordConfigData);
    if (PasswordConfigData.UserPasswordEnable == 1 || PasswordConfigData.AdminPasswordEnable == 1) {
      while (1) {
        ...
        if (PasswordConfigData.UserPasswordEnable == 1 && StrCmp (TempStr, PasswordConfigData.UserPassword) == 0) {
          if (PasswordConfigData.AdminPasswordEnable == 1) {
            UserPriv = 1;
          }
          break;
        } else if (PasswordConfigData.AdminPasswordEnable == 1 && StrCmp (TempStr, PasswordConfigData.AdminPassword) == 0) {
          UserPriv = 0;
          break;
        } else {
          //incorrect
...
/**
  ...
  @param[in]  HiiHandle       Points to the hii handle for this formset.
  ...
  @param[out] Status          Return the handle status.
  ...
  )
{
  if (Action == EFI_BROWSER_ACTION_FORM_OPEN) {
    if (mFirstEnterFrontPage == 0 && QuestionId == 0x809) {
      mFirstEnterFrontPage = TRUE;
      FrontPagePasswordCheck ();
    }
  }
  ...
  //add for F9  LoadDefault function
  if (Action == EFI_BROWSER_ACTION_DEFAULT_STANDARD) {
    if (QuestionId == FRONT_PAGE_KEY_LANGUAGE) {
      //DEBUG((EFI_D_ERROR,"EFI_BROWSER_ACTION_DEFAULT_STANDARD>FRONT_PAGE_KEY_LANGUAGE %d \n",gDefaultLanguageIndex));
      //en-US;zh-Hans
      // 0   1 
      //Value->u8 = 1;
      Value->u8 = gDefaultLanguageIndex;
  ...
  )
{
  ...
  //zhjmxl add for F9=Reset to Defaults,record the PcdUefiVariableDefaultPlatformLang
  DefaultLang = AllocateCopyPool (
                               AsciiStrSize ((CHAR8 *) PcdGetPtr (PcdUefiVariableDefaultPlatformLang)),
                               (CHAR8 *) PcdGetPtr (PcdUefiVariableDefaultPlatformLang)
                               );
  ASSERT (DefaultLang != NULL);
  ...
  while (*LangCode != 0) {
    ...
    if (DefaultLang != NULL && AsciiStrCmp (Lang, DefaultLang) == 0) 
    {
       //DEBUG((EFI_D_ERROR,"UiCreateLanguageMenu 1-2 %d %s\n",OptionCount,DefaultLang));
       gDefaultLanguageIndex = (UINT8) OptionCount;
    }
...
/**
  Create PowerOff menu in the front page.
  ...
  )
{
  HiiCreateActionOpCode (
    ...
    STRING_TOKEN (STR_SHUTDOWN_STRING),
    STRING_TOKEN (STR_SHUTDOWN_STRING_HELP),
    EFI_IFR_FLAG_CALLBACK,
    0
    );
}
...
/**
  ...
  @param[in]    HiiHandle           The hii handle for the driver.
  @param[in]    Guid                The special guid for the driver which is the target.
  @param[out]   PromptId            Return the prompt string id.
  @param[out]   HelpId              Return the help string id.
  @param[out]   FormsetGuid         Return the formset guid info.
...
/**
  ...
  @param[in]    HiiHandle           The hii handle for the Uiapp driver.
  @param[in]    ClassGuid           The class guid for the driver which is the target.
  @param[in]    SpecialHandlerFn    The pointer to the specail handler function, if any.
  @param[in]    StartOpCodeHandle   The opcode handle to save the new opcode.
