...
DATE_TIME_PRIVATE_DATA      *mPrivateData = NULL;
  ...
  )
{
  ...
  NewString        = NULL;
  mPrivateData = AllocateZeroPool (sizeof (DATE_TIME_PRIVATE_DATA));
  if (mPrivateData == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  mPrivateData->Signature = SET_DATETIME_PRIVATE_SIGNATURE;
  ...
  //
  NewString = ConvertDevicePathToText((EFI_DEVICE_PATH_PROTOCOL*)&mSetDateAndTimeHiiVendorDevicePath, FALSE, FALSE);
  if (HiiSetString (HiiHandle, STRING_TOKEN (STR_DEVICE_PATH), NewString, NULL) == 0) {
    SetDateAndTimeUnload (ImageHandle);
    return EFI_OUT_OF_RESOURCES;
  }
  if (NewString != NULL) {
    FreePool (NewString);
