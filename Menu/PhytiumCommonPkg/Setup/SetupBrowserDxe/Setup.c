  ...
  )
{
  ...
  if (Storage == NULL) {
	...
	//zhjmxl EfiTime.Nanosecond should < 999999999,otherwise gRT->SetTime() will error
  ...
  )
{
  ...
  QUESTION_OPTION         *Option;
  //zhjmxl add
  QUESTION_OPTION         *OptionAll;
  UINT8                   MaxOptionCount = 100; 
  UINT8                   IndexCount = 0;
  UINT8                   i = 0;
  UINT8                   j = 0;
  ...
  OptionAll    = AllocatePages (EFI_SIZE_TO_PAGES (MaxOptionCount * sizeof (*OptionAll))); 
  //
...
ReGetDefault:
  ...
  case EFI_IFR_ORDERED_LIST_OP:
    ...
    //
    //use the old code to record the data
    Index = 0;
    Link = GetFirstNode (&Question->OptionListHead);
    while (!IsNull (&Question->OptionListHead, Link)) {
      Status = EFI_SUCCESS;
      ...
      //OptionAll[Index].Value.Value.u64 = Option->Value.Value.u64;
      OptionAll[Index] = *Option;
      //DEBUG ((DEBUG_INFO, "Option.Value %d\n",OptionAll[Index].Value.Value.u64));
      Index++;
      IndexCount++;
    ...
    //ReArry the record data,litter to big 
    for ( i = 0; i < IndexCount-1; i++)
    {
        // 具体冒泡的方式：用相邻的两个元素进行比较，前一个大于后一个元素时，交换着两个数据，依次直到数组的末尾
        for ( j = 1; j < IndexCount-i; j++)
        {
            if (OptionAll[j-1].Value.Value.u64 > OptionAll[j].Value.Value.u64)
            {
                //temp = array[j - 1];
                //array[j - 1] = array[j];
                //array[j] = temp;
		//Use Index99 as temp
		OptionAll[99].Value.Value.u64 = OptionAll[j-1].Value.Value.u64;
		OptionAll[j-1].Value.Value.u64 = OptionAll[j].Value.Value.u64;
		OptionAll[j].Value.Value.u64 = OptionAll[99].Value.Value.u64;
            }
        }
    }
    //Old Code 
    ...
    while (!IsNull (&Question->OptionListHead, Link)) {
      ...
      //ReArray with litter to big
      SetArrayData (Question->BufferValue, Question->ValueType, Index, OptionAll[Index].Value.Value.u64);
  ...
  FreePages (OptionAll, EFI_SIZE_TO_PAGES (MaxOptionCount * sizeof (*OptionAll)));
  return Status;
}
