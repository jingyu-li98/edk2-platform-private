...
@param[in]      Value             A pointer to the data being complexity verificied.
...
{
   UINTN                          Index;
   BOOLEAN                        PasswordWithNumber = FALSE;
   BOOLEAN                        PasswordWithUpperCase = FALSE;
   BOOLEAN                        PasswordWithLowerCase = FALSE;
   BOOLEAN                        PasswordWithSpecialChar = FALSE;

   EFI_STATUS        Status;
   UINTN             VarSize;
   UINT8             UserPriv;

   DEBUG((EFI_D_INFO, "LXQ PasswordComplexityCheck begin.\n"));
   Status = EFI_SUCCESS;
   ...
   if (StrLen(Value) > PASSWD_MAXLEN_ALLOW){
        return EFI_INVALID_PARAMETER;
   }
   ...
   for (Index=0; Index < StrLen(Value);  Index++){
           ...
           }else if ((Value[Index] >= L'A') && (Value[Index] <= L'Z')){
                        PasswordWithUpperCase = TRUE;
           }else if ((Value[Index] >= L'a') && (Value[Index] <= L'z')){
                        PasswordWithLowerCase = TRUE;
           }else {
                        PasswordWithSpecialChar = TRUE;
           }
   }
   DEBUG((EFI_D_INFO, "LXQ PasswordComplexityCheck end.\n"));
   if ((PasswordWithNumber && PasswordWithUpperCase) || (PasswordWithNumber && PasswordWithLowerCase) || (PasswordWithNumber && PasswordWithSpecialChar) || (PasswordWithUpperCase && PasswordWithLowerCase) || (PasswordWithUpperCase && PasswordWithSpecialChar) || (PasswordWithLowerCase && PasswordWithSpecialChar)){
                return EFI_SUCCESS;
   }else{
                return EFI_INVALID_PARAMETER;
   }
  ...
  )
{
  ...
  *StringPtr = 0;
  ...
  // LXQ check password complexity. { 
  Status = PasswordComplexityCheck(StringPtr);
  if (EFI_ERROR (Status)) {
    Question->PasswordCheck (gFormData, Question, NULL);
    do {
      CreateDialog (&Key, gEmptyString, gPasswordAsRequired, gPressEnter, gEmptyString, NULL);
    } while (Key.UnicodeChar != CHAR_CARRIAGE_RETURN);
    FreePool (StringPtr);
    return Status;
  }
