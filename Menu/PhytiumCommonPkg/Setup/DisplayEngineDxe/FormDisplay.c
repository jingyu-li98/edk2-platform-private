...
CHAR16            *gPasswordAsRequired;
CHAR16            *gUserAccessDeny;
  ...
  )
{
  ...
  gPasswordAsRequired   = GetToken (STRING_TOKEN (PLEASE_ENTER_AS_REQUIRED), gHiiHandle);
  gUserAccessDeny       = GetToken (STRING_TOKEN (USER_ACCESS_DENY), gHiiHandle);
}
  ...
  )
{
  ...
  FreePool (gPasswordAsRequired);
  FreePool (gUserAccessDeny);
}
  ...
  )
{
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    //Save the org MenuOption->GrayOut
    OldGrayOut = MenuOption->GrayOut;
    ...
    case EFI_IFR_STRING_OP:
      ...
      if (Status == EFI_NOT_FOUND) {
        DEBUG((EFI_D_ERROR, "FormDisply hasn't password.\n"));
	MenuOption->GrayOut = FALSE;
      } else {
        if (UserPriv == 0x1) {
          MenuOption->GrayOut = TRUE;
...
/**
  Count the storage space of a Unicode string,with wide char.
**/
  ...
  )
{
  ...
  do {
    ...
    for (;(String[Index] != NARROW_CHAR) && (String[Index] != WIDE_CHAR) && (String[Index] != 0);Index++){
      if (String[Index]>=0x4e00 && String[Index]<=0x9fa4){
        IncrementValue = 2;
...
{
  UINTN            Length;
  //DEBUG((DEBUG_INFO,"%a(),%d,Row:%d\n",__FUNCTION__,__LINE__,Row));
  //DEBUG((DEBUG_INFO,"%a(),%d,Col:%d\n",__FUNCTION__,__LINE__,Col));
  //DEBUG((DEBUG_INFO,"%a(),%d,Width:%d\n",__FUNCTION__,__LINE__,Width));
  //
  ...
  //
  //Recount the string length with chinese char
  Length=(GetStringWidthNW (String) / 2) - 1;  // -1 is remove null
  ...
  )
{
  ...
  Status = gRT->GetVariable (
   ...
   {
     DEBUG((EFI_D_ERROR, "zhjmxl UserPriv is %d\n",UserPriv));
   }
  ...
  {
      do {
        CreateDialog (&Key, gEmptyString, gUserAccessDeny, gEmptyString, NULL);
  ...
  )
{
  ...
  //zhjmxl_F9,according language reload help strings
  InitializeDisplayStrings();
  ...
  while (TRUE) {
    ...
    case CfInitialization:
      //DEBUG((DEBUG_INFO,"case CfInitialization\n"));
    ...
    case CfRepaint:
      ...
      //DEBUG((DEBUG_INFO,"case CfRepaint\n"));
      if (Repaint) {
        //
        // Display menu
        //
    ...
    case CfRefreshHighLight:
      //DEBUG((DEBUG_INFO,"case CfRefreshHighLight\n"));
      //
    ...
    case CfUpdateHelpString:
      //DEBUG((DEBUG_INFO,"case CfUpdateHelpString\n"));
    ...
    case CfPrepareToReadKey:
      //DEBUG((DEBUG_INFO,"case CfPrepareToReadKey\n"));
      ControlFlag = CfReadKey;
      ScreenOperation = UiNoOperation;
      break;

    case CfReadKey:
      //DEBUG((DEBUG_INFO,"case CfReadKey\n"));
    ...
    case CfScreenOperation:
      //DEBUG((DEBUG_INFO,"case CfScreenOperation\n"));
    ...
    case CfUiSelect:
      //DEBUG((DEBUG_INFO,"case CfUiSelect\n"));
      ControlFlag = CfRepaint;
    ...
    case CfUiReset:
      //DEBUG((DEBUG_INFO,"case CfUiReset\n"));
      //
    ...
    case CfUiHotKey:
      //DEBUG((DEBUG_INFO,"case CfUiHotKey\n"));
      ControlFlag = CfRepaint;
    ...
    case CfUiLeft:
      //DEBUG((DEBUG_INFO,"case CfUiLeft\n"));
      ControlFlag = CfRepaint;
      ASSERT(MenuOption != NULL);
    ...
    case CfUiRight:
      //DEBUG((DEBUG_INFO,"case CfUiRight\n"));
      ControlFlag = CfRepaint;
      ASSERT(MenuOption != NULL);
      if ((MenuOption->ThisTag->OpCode->OpCode == EFI_IFR_DATE_OP) || (MenuOption->ThisTag->OpCode->OpCode == EFI_IFR_TIME_OP)) {
    ...
    case CfUiPageUp:
      //DEBUG((DEBUG_INFO,"case CfUiPageUp\n"));
      //
    ...
    case CfUiPageDown:
      //DEBUG((DEBUG_INFO,"case CfUiPageDown\n"));
      //
      // SkipValue means lines is skipped when show the top menu option.
      //
      ControlFlag = CfRepaint;
    ...
    case CfUiDown:
      //DEBUG((DEBUG_INFO,"case CfUiDown\n"));
      //
      // SkipValue means lines is skipped when show the top menu option.
    ...
    case CfUiNoOperation:
      //DEBUG((DEBUG_INFO,"case CfUiNoOperation\n"));
      ControlFlag = CfRepaint;
      break;

    case CfExit:
      //DEBUG((DEBUG_INFO,"case CfExit\n"));
