...
STATIC EFI_EVENT  mI3cRuntimeVirtualAddrChangeEvent;
STATIC UINTN      mI3cBaseAddress;
...
{
  MmioWrite32 (mI3cBaseAddress + CTRL, CTRL_DEV_DIS);
}
  ...
  )
{
  ...
  while (!(MmioRead32 (mI3cBaseAddress + MST_STATUS0) & MST_STATUS0_IDLE) ||
    !(MmioRead32 (mI3cBaseAddress + MST_STATUS0) & MST_STATUS0_CMDD_EMP)) {
  ...
  )
{
  ...
  if (Bus->TransMode == I3C_MODE) {
    MmioWrite32 (mI3cBaseAddress + DEV_ID_RR0 (1), (DevIDxRR0 | DEV_ID_RR0_IS_I3C));
  } else {
    MmioWrite32 (mI3cBaseAddress + DEV_ID_RR0 (1), DevIDxRR0);
  }
  ...
  )
{
  ...
  MmioWrite32 (mI3cBaseAddress + CTRL, Ctrl);
  ...
  for (Index1 = 0; Index1 < Xfer->Number; Index1++) {
    ...
    for (Index2 = 0; Index2 < WriteCount; Index2++) {
      ...
      //
      while (MmioRead32 (mI3cBaseAddress + MST_STATUS0) & MST_STATUS0_TX_FULL) {
        MicroSecondDelay (I3C_DELAY);
        if (Timeout-- == 0) {
      ...
      MmioWrite32 (mI3cBaseAddress + TX_FIFO, TxFiFo[Index2]);
    }

    MmioWrite32 (mI3cBaseAddress + CMD1_FIFO, Xfer->Cmds[Index1].Cmd1);
    MmioWrite32 (mI3cBaseAddress + CMD0_FIFO, Xfer->Cmds[Index1].Cmd0);
    ...
    if (Xfer->Cmds[Index1].Cmd0 & CMD0_FIFO_RNW) {
      ...
      //
      while ((MmioRead32 (mI3cBaseAddress + MST_STATUS0) & MST_STATUS0_RX_EMP) ||
        !(MmioRead32 (mI3cBaseAddress + MST_STATUS0) & MST_STATUS0_IDLE)) {
        MicroSecondDelay (I3C_DELAY);
        if (Timeout-- == 0) {
      ...
      for (Index2 = 0; Index2 < ReadCount; Index2++) {
        RxFiFo[Index2] = MmioRead32 (mI3cBaseAddress + RX_FIFO);
      }

      CopyMem (Xfer->Cmds[Index1].RxBuf, RxFiFo, Xfer->Cmds[Index1].RxLen);

      while (!(MmioRead32 (mI3cBaseAddress + MST_STATUS0) & MST_STATUS0_RX_EMP)) {
        MmioRead32 (mI3cBaseAddress + RX_FIFO);
      }
    }
  }
  ...
  )
{
  ...
  SpdI3cPrivexfer.Rnw = CMD0_FIFO_RNW;
  SpdI3cPrivexfer.Lenth = Length;
  SpdI3cPrivexfer.DataIn = Buffer;
  ...
  )
{
  ...
  if (Bus->TransMode == I3C_MODE) {
    if (Bus->SubSCBA == SubAddress8Bit) {
      ...
      SpdI3cPrivexfer.Lenth = (Length + 1);
        ...
        SpdI3cPrivexfer.Lenth = (Length + 2);
    }
  ...
  SpdI3cPrivexfer.DataOut = TxBuffer;
...
VOID
EFIAPI
LibI3cRuntimeVirtualNotifyEvent (
  IN EFI_EVENT        Event,
  IN VOID             *Context
  )
{
  ...
  EfiConvertPointer (0x0, (VOID**)&mI3cBaseAddress);
  ...
  )
{
  ...
  // Initialize I3c Runtime Base Address

  mI3cBaseAddress   = PcdGet64 (PcdI3cRuntimeBaseAddress);
  ...
  Status = gBS->CreateEventEx (
                  ...
                  &mI3cRuntimeVirtualAddrChangeEvent
                  );
