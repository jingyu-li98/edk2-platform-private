...
#define MAX_NOR_FLASH_INSTANCES 2

SPI_MASTER *mSpiMasterInstance;
NOR_FLASH_INSTANCE *mNorFlashInstances[MAX_NOR_FLASH_INSTANCES];
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: Failed to locate FDT client protocol: %r\n",
      __func__,
      Status
      ));
                                     ...
                                     ))
  {
    ...
    if (EFI_ERROR (Status)) {
      ...
      Nor[Number]->SpiBase = SwapBytes64 (((CONST UINT64 *) RegProp)[0]);
    }
  ...
  for (Index = 0; Index < Number; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
        "%a: Failed to set memory attributes for instance %d\n",
        __func__,
	Index
        ));
    ...
    Nor[Index]->BounceBufSize = SIZE_4KB;
    Nor[Index]->BounceBuf = AllocateZeroPool (Nor[Index]->BounceBufSize);
    if (!Nor[Index]->BounceBuf) {
      Nor = NULL;
      goto Error;
    }

    SpifmcInit (&Nor[Index]);

    DEBUG ((
      DEBUG_VERBOSE,
      "%a[%d] SPI Base Address for instance %d = 0x%llx\n",
      __func__,
      __LINE__,
      Index,
      Nor[Index]->SpiBase
