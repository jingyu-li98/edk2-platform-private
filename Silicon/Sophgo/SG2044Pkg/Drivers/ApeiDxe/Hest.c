  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a: Starting PCIe RC configuration parsing\n", __func__));
  ...
  for (FindNodeStatus = FdtClient->FindCompatibleNode (FdtClient, "sophgo,sg2044-pcie-host", &Node);
    ...
    DEBUG ((DEBUG_INFO, "%a: Found RC%d: enabled=%d, port_count=%d, config_base=0x%lx\n",
            __func__, 
  ...
  DEBUG ((DEBUG_INFO, "%a: Final PCIe RC count: %d\n", __func__, mPcieRcCount));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "  Error Status Block: 0x%lx\n", 
          GhesV2->ErrorStatusAddress.Address));
  DEBUG ((DEBUG_INFO, "  Read Ack Register (DBI): 0x%lx\n", 
          GhesV2->ReadAckRegister.Address));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a: DDR ECC error source initialized for Controller %d at 0x%lx\n",
  ...
  )
{
  ...
  for (Index = 0; Index < mPcieRcCount; Index++) {
    ...
    DEBUG ((DEBUG_INFO, "%a: Added PCIe error source %d with ID 0x%x\n",
            __func__, ValidSourceCount, GhesV2[ValidSourceCount].SourceId));

    CurrentBlock = (UINT8 *)CurrentBlock + GENERIC_HARDWARE_ERROR_BLOCK_SIZE;
    ValidSourceCount++;
  }
  ...
  for (Index = 0; Index < DDR_CFG_BASE_ARRAY_SIZE * 2; Index++) {
    ...
    DEBUG ((DEBUG_INFO, "%a: Added DDR error source %d with ID 0x%x\n",
            __func__, ValidSourceCount, GhesV2[ValidSourceCount].SourceId));
  ...
  DEBUG ((DEBUG_INFO, "%a: Initialized %d valid error sources out of %d total\n",
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a: Returning total error sources: PCIe RC=%d + DDR=%d = %d\n",
