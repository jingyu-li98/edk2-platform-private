  ...
  )
{
  ...
  BOOLEAN WasSpace;
  WasSpace = FALSE;
  while (*String && (!Chars || !strchr (Chars, *String)) &&
		!(WasSpace && strchr (INI_INLINE_COMMENT_PREFIXES, *String))) {
    WasSpace = IsSpace(*String);
    String ++;
  }
#else
  while (*String && (!Chars || !strchr (Chars, *String))) {
  ...
  )
{
  ...
  Ctx = (INI_PARSE_STRING_CONTEXT *)Stream;
  CtxPtr = Ctx->Ptr;
