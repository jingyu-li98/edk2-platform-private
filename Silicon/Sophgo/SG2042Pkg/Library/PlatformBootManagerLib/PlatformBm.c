  ...
  )
{
  ...
  UINTN          Ret;
  UINTN          Index;
  UINTN          StartFlashAddr;

  NorFlash = NULL;
  Index = 0;
  DataSize = SIZE_16KB;
  // StartFlashAddr = SIZE_32MB;
  StartFlashAddr = 0x0;
  ...
  // Allocate buffers
  //
  ReadBuffer = AllocateZeroPool (DataSize);
  if (!ReadBuffer) {
    Status = EFI_OUT_OF_RESOURCES;
    goto EXIT;
  }

  WriteBuffer = AllocateZeroPool (DataSize);
  if (!WriteBuffer) {
    Status = EFI_OUT_OF_RESOURCES;
    goto EXIT;
  }
  ...
  //
  SetMem(WriteBuffer, DataSize, 0xCC);

  //
  // Write Data
  //
  Status = NorFlashProtocol->WriteData (NorFlash, StartFlashAddr, DataSize,
		                       (UINT8 *)WriteBuffer);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: Write data into Nor Flash failed!\n",
      __func__
      ));
    goto EXIT;
  }
  ...
  // Read Data
  //
  Status = NorFlashProtocol->ReadData (NorFlash, StartFlashAddr, DataSize,
		                      (UINT8 *)ReadBuffer);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  //
  Ret = CompareMem(WriteBuffer, ReadBuffer, DataSize);
  if (Ret != 0) {
    Status = EFI_DEVICE_ERROR;
    DEBUG ((
      DEBUG_ERROR,
      "\n\n%a[%d] !!! Read/Write Compare result = 0x%X\n -----\n",
      __func__,
      __LINE__,
      Ret));
  ...
  //
  Status = NorFlashProtocol->EraseChip (NorFlash);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: Erase the whole Nor Flash failed!\n",
      __func__
      ));
    goto EXIT;
  }
#endif
  DEBUG ((DEBUG_WARN, "%a[%d]: Start Erase Test\n", __func__, __LINE__));
  //
  // Initialize WriteBuffer
  //
  SetMem(WriteBuffer, DataSize, 0xFF);
#if 1
  //
  // Erase Data
  //
  Status = NorFlashProtocol->Erase (NorFlash, StartFlashAddr, DataSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: Erase Nor Flash device failed!\n",
      __func__
      ));
    goto EXIT;
  }
  ...
  //
  // Verify that the flash region is erased
  //
  Ret = CompareMem(WriteBuffer, ReadBuffer, DataSize);
  if (Ret != 0) {
    Status = EFI_DEVICE_ERROR;
