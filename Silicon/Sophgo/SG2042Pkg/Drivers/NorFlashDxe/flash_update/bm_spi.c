	...
	uint32_t with_cmd, uint32_t addr_bytes)
{
	...
	if (addr_bytes > 3)
		mem_rw_ops.write32(spi_base + REG_SPI_FIFO_PORT, p_data[1]);
	...
	uint32_t with_cmd, uint32_t addr_bytes, uint32_t data_bytes)
{
	...
	if (with_cmd) {
		if (addr_bytes > 3) {
			mem_rw_ops.write8(spi_base + REG_SPI_FIFO_PORT, cmd_buf[0]);
			mem_rw_ops.write8(spi_base + REG_SPI_FIFO_PORT, cmd_buf[1]);
			mem_rw_ops.write8(spi_base + REG_SPI_FIFO_PORT, cmd_buf[2]);
			mem_rw_ops.write8(spi_base + REG_SPI_FIFO_PORT, cmd_buf[3]);
			mem_rw_ops.write8(spi_base + REG_SPI_FIFO_PORT, cmd_buf[4]);
		} else {
			for (i = 0; i < ((cmd_bytes - 1) / 4 + 1); i++)
				mem_rw_ops.write32(spi_base + REG_SPI_FIFO_PORT, p_data[i]);
		}
	...
	while ((mem_rw_ops.read32(spi_base + REG_SPI_FIFO_PT) & 0xF) != 0)
		; //wait for cmd issued
	if (addr_bytes > 3)
		mem_rw_ops.write32(spi_base + REG_SPI_FIFO_PT, 0); // flush FIFO
	/* fill data */
...
static uint8_t spi_enable_4byte_mode(uint64_t spi_base)
{
	uint8_t cmd_buf[5];

	memset(cmd_buf, 0, sizeof(cmd_buf));
	cmd_buf[0] = SPI_CMD_ENABLE_4BYTE_MODE; // enable 4-Byte Address Mode
	spi_non_data_tran(spi_base, cmd_buf, 1, 0);

	return 0;
}

static uint8_t spi_disable_4byte_mode(uint64_t spi_base)
{
	uint8_t cmd_buf[5];

	memset(cmd_buf, 0, sizeof(cmd_buf));
	cmd_buf[0] = SPI_CMD_DISABLE_4BYTE_MODE; // disable 4-Byte Address Mode
	spi_non_data_tran(spi_base, cmd_buf, 1, 0);
...
static int spi_data_read(uint64_t spi_base, uint8_t *dst_buf, int addr, int size)
{
	...
	cmd_buf[0] = SPI_CMD_READ; // Read Data Bytes
...
static int spi_4byte_data_read(uint64_t spi_base, uint8_t *dst_buf, int addr, int size)
{
	uint8_t cmd_buf[5];

	cmd_buf[0] = SPI_CMD_4BYTE_READ; // 4-Byte Read Data Bytes
...
size_t spi_flash_read_blocks(int lba, uint8_t *buf, size_t size)
{
...
static uint8_t spi_read_status(uint64_t spi_base)
{
	...
	cmd_buf[0] = SPI_CMD_RDSR; // read status register
...
static uint8_t spi_page_program(uint64_t spi_base, uint8_t *src_buf, uint32_t addr, uint32_t size)
{
	...
	cmd_buf[0] = SPI_CMD_PP; // Page Program
	cmd_buf[1] = (addr >> 16) & 0xFF;
	cmd_buf[2] = (addr >> 8) & 0xFF;
	cmd_buf[3] = addr & 0xFF;
...
static uint8_t spi_4byte_page_program(uint64_t spi_base, uint8_t *src_buf,
				   uint32_t addr, uint32_t size)
{
	...
	cmd_buf[0] = SPI_CMD_4BYTE_PP; // 4-Byte Page Program
	cmd_buf[1] = (addr >> 24) & 0xFF;
	cmd_buf[2] = (addr >> 16) & 0xFF;
	cmd_buf[3] = (addr >> 8) & 0xFF;
	cmd_buf[4] = addr & 0xFF;

	spi_data_out_tran(spi_base, src_buf, cmd_buf, 1, 4, size);
	return 0;
}
...
static void spi_sector_erase(uint64_t spi_base, uint32_t addr)
{
	uint8_t cmd_buf[4];
	memset(cmd_buf, 0, sizeof(cmd_buf));
	cmd_buf[0] = SPI_CMD_SE;  // 64KB Block Erase
	cmd_buf[1] = (addr >> 16) & 0xFF;
	cmd_buf[2] = (addr >> 8) & 0xFF;
	cmd_buf[3] = addr & 0xFF;
...
static void spi_4byte_sector_erase(uint64_t spi_base, uint32_t addr)
{
	uint8_t cmd_buf[5];

	memset(cmd_buf, 0, sizeof(cmd_buf));
	cmd_buf[0] = SPI_CMD_4BYTE_SE; // 4-Byte 64KB Block Erase
	cmd_buf[1] = (addr >> 24) & 0xFF;
	cmd_buf[2] = (addr >> 16) & 0xFF;
	cmd_buf[3] = (addr >> 8) & 0xFF;
	cmd_buf[4] = addr & 0xFF;

	spi_non_data_tran(spi_base, cmd_buf, 1, 4);
}

void bm_spi_init(uint64_t base, int len)
{
	uint32_t tran_csr = 0;
	...
	if (len > 16777216) { /* file size > 16MB */
		spi_enable_4byte_mode(mem_rw_ops.ctrl_base);
		tran_csr |= (REG_SPI_CE_CTRL << SPI_TRAN_CSR_ADDR_BYTES_SHIFT);
	} else
...
void bm_spi_uninit(int len)
{
	if (len > 16777216) /* file size > 16MB */
		spi_disable_4byte_mode(mem_rw_ops.ctrl_base);
...
int bm_spi_flash_erase_sector(uint64_t spi_base, uint32_t addr, int len)
{
	uint32_t spi_status, wait = 0;

	spi_write_en(spi_base); /* Write Enable */
	...
	if (len > 16777216)
		spi_4byte_sector_erase(spi_base, addr);
	else
...
static int do_page_program(uint64_t spi_base, uint8_t *src_buf, uint32_t addr, uint32_t size, int len)
{
	uint8_t spi_status;
	uint32_t wait = 0;
	...
	if (len > 16777216)
		spi_4byte_page_program(spi_base, src_buf, addr, size);
	else
...
static void fw_sp_read_test(uint64_t spi_base, uint32_t addr, int len)
{
	unsigned char cmp_buf[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
	if (len > 16777216)
		spi_4byte_data_read(spi_base, cmp_buf, addr, sizeof(cmp_buf));
	else
...
{
	uint32_t xfer_size, cmp_ret;
	uint64_t off;
	/* each block: 256 pages */
	uint8_t cmp_buf[SPI_FLASH_BLOCK_SIZE];
	uint32_t id, sector_size, erased_sectors;
	...
	for (int i = 0; i < erased_sectors; i++)
		bm_spi_flash_erase_sector(mem_rw_ops.ctrl_base, base + i * sector_size, size);

	printf("erase OK\n");
	fw_sp_read_test(mem_rw_ops.ctrl_base, base, size);
	...
	while (off < size) {
		...
		if (do_page_program(mem_rw_ops.ctrl_base, src_buf + off, base + off, xfer_size, size) != 0) {
			ERROR("page prog failed @ 0x%lx\n", base + off);
			return -1;
		}
		if (size > 16777216)
			spi_4byte_data_read(mem_rw_ops.ctrl_base, cmp_buf, base + off, xfer_size);
		else
		...
		VERBOSE("page read compare ok @ %ld\n", off);
		off += xfer_size;
		printf("\b\b\b%2ld%%", off * 100 / size);
	}
	printf("\n");
