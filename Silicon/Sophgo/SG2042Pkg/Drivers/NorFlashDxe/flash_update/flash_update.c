...
int read_bin_from_flash(char *buf, unsigned long spi_addr_phy, uint32_t offset, int len)
{
	...
	while (len > 0) {
		rds = len > SPI_FLASH_BLOCK_SIZE ? SPI_FLASH_BLOCK_SIZE : len;

		spi_flash_read_blocks(offset, buf, SPI_FLASH_BLOCK_SIZE);
...
int set_write_protection(unsigned long spi_addr_phy, int enable)
{
	...
	if (!spi_base)
		return -ENOMEM;
	bm_spi_init((unsigned long)spi_addr_phy, len);
...
int main(int argc, char **argv)
{
	...
	if (argc == 1) {
		...
		printf("SG2042 X8EVB update spi_flash.bin in SPI Flash1:\n\t");
		printf("./flash_update -f spi_flash.bin -b 0x7002180000\r\n");
		printf("\n");

		printf("SG2042 X8EVB dump spi_flash.bin from SPI Flash1:\n\t");
		printf("./flash_update -d spi_flash_dump.bin -b 0x7002180000 -o 0 -l 0x1000000\r\n");
		printf("\n");
