  ...
  )
{
  ...
  if (BmParams.Flags & SD_USE_PIO) {
    ...
    BlockSize /= 4;
      DEBUG ((DEBUG_WARN, "%a[%d] INT_STATUS(0x30)=0x%lx\tErrstatus(0x32)=0x%lx\tMmioRead32 (Base + SDHCI_PSTATE)=0x%lx\n", __func__, __LINE__, MmioRead16 (Base + SDHCI_INT_STATUS), MmioRead16 (Base + SDHCI_ERR_INT_STATUS), MmioRead32 (Base + SDHCI_PSTATE)));
    ...
    Timeout = 0;
    DEBUG ((DEBUG_WARN, "%a[%d] MmioRead16 (Base + SDHCI_INT_STATUS)=0x%lx\tErrstatus=0x%lx\tMmioRead32 (Base + SDHCI_PSTATE)=0x%lx\n", __func__, __LINE__, MmioRead16 (Base + SDHCI_INT_STATUS), MmioRead32 (Base + SDHCI_PSTATE), MmioRead16 (Base + SDHCI_ERR_INT_STATUS)));
    while (1) {
      Status = MmioRead16 (Base + SDHCI_INT_STATUS);
