  ...
  )
{
  ...
  RootBridge->RootBridgeIo.FreeBuffer     = NonCoherentRootBridgeIoFreeBuffer;
  ...
  )
{
  ...
  return mCpuIo->Io.Read (
                      ...
                      TO_HOST_ADDRESS (Address, RootBridge->Mem.Translation),
                      Count,
                      Buffer
                      );
}
  ...
  )
{
      ...
      ((Attributes & EFI_PCI_ATTRIBUTE_DUAL_ADDRESS_CYCLE) == 0))
  {
    ...
    //
    DEBUG ((DEBUG_WARN, "!!!Limit allocations to memory below 4GB\n"));
  ...
  )
{
  ...
  EFI_STATUS                                   Status;
  UINT64                                       MemType;
  PCI_ROOT_BRIDGE_UNCACHED_INSTANCE            *Alloc;
  VOID                                         *AllocAddress;
  ...
  Status = RootBridgeIoAllocateBuffer (
            This,
            Type,
            MemoryType,
            Pages,
  ...
  //
  InsertHeadList (&RootBridge->Link, &Alloc->Link);
...
FreeBuffer:
  RootBridgeIoFreeBuffer (This, Pages, AllocAddress);
  return Status;
}
...
EFI_STATUS
EFIAPI
NonCoherentRootBridgeIoFreeBuffer (
  IN  EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL  *This,
  IN  UINTN                            Pages,
  IN  VOID                             *HostAddress
  )
{
  ...
  EFI_STATUS                                   Status;
  PCI_ROOT_BRIDGE_UNCACHED_INSTANCE            *Alloc;
  PCI_ROOT_BRIDGE_UNCACHED_INSTANCE            *AllocHead;
  PCI_ROOT_BRIDGE_UNCACHED_INSTANCE            *AllocTail;
  BOOLEAN                                      Found;
  UINTN                                        StartPages;
  UINTN                                        EndPages;
  ...
  //
  for (Entry = RootBridge->Link.ForwardLink;
       Entry != &RootBridge->Link;
       Entry = Entry->ForwardLink)
  {
    Alloc = BASE_CR (Entry, PCI_ROOT_BRIDGE_UNCACHED_INSTANCE, Link);
  ...
  //
  if (AllocHead != NULL) {
    InsertHeadList (&RootBridge->Link, &AllocHead->Link);
  }
  ...
  if (!Bounce) {
    switch (Operation) {
      ...
      case EfiPciOperationBusMasterWrite64:
        ...
        if (((PhysicalAddress | *NumberOfBytes) & AlignMask) == 0) {
          break;
        }
